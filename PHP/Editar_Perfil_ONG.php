...
try {
    ...
    $query = "SELECT nome, email_ong, cnpj, telefone, cep, endereco, endereco_numero, descricao, cebas FROM ongs WHERE id_ong = ?";
...
if ($cebas && $cebas['error'] === UPLOAD_ERR_OK) {
    ...
    move_uploaded_file($cebas['tmp_name'], $uploadPath);
}
        ...
        $update_query = "UPDATE ongs SET nome = ?, email_ong = ?, cnpj = ?, telefone = ?, cep = ?, endereco = ?, endereco_numero = ?, descricao = ?, cebas = ? WHERE id_ong = ?";
        $update_stmt = $conn->prepare($update_query);
        $update_stmt->bind_param("sssssssssi", $nome, $email, $cnpj, $telefone, $cep, $endereco, $endereco_numero, $descricao, $cebasName, $id_ong);
...
<body>
    ...
    <main class="container ">
        ...
        <form method="POST" action="Editar_Perfil_ong.php" enctype="multipart/form-data">
            <div class="divimg d-flex" onclick="abrirArquivo()">
                
                <img src="./uploads/<?php echo htmlspecialchars($user['cebas']); ?>" class="imgong" alt="">
                <i class="bi bi-gear-fill"></i>
            </div>
            <input type="file" name="cebas" id="arquivo" hidden>
            <div class="row">
            <div class="col mb-6">
